CREATE database if NOT EXISTS `nacha_deployer` default character set utf8 collate utf8_general_ci;
...
CREATE TABLE `project_core` (
  ...
  `project_description` text COLLATE utf8_bin COMMENT '描述',
  ...
  `env_name` varchar(255) COLLATE utf8_bin DEFAULT NULL,
  `partition_id` int(11) DEFAULT NULL COMMENT '分区id',
  `project_type` varchar(200) COLLATE utf8_bin NOT NULL COMMENT '应用类型(通用、容器、springcloud、grpc、dubbo)',
  `network_name` varchar(200) COLLATE utf8_bin DEFAULT NULL COMMENT 'fabric网络名称',
  `partition_name` varchar(200) COLLATE utf8_bin DEFAULT NULL COMMENT '分区名称',
  PRIMARY KEY (`project_id`),
  UNIQUE KEY `project_index` (`project_name`,`cluster_id`,`project_current_status`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COLLATE=utf8_bin;
...
CREATE TABLE `v2_application` (
  ...
  `micro_service_status` varchar(200) COLLATE utf8_bin DEFAULT NULL,
  `micro_service_type` varchar(200) COLLATE utf8_bin DEFAULT NULL,
  `monitor_status` varchar(200) COLLATE utf8_bin DEFAULT NULL,
  `apollo_status` varchar(200) COLLATE utf8_bin DEFAULT NULL,
  `kind` varchar(200) COLLATE utf8_bin DEFAULT NULL,
  `schedule` varchar(200) COLLATE utf8_bin DEFAULT NULL,
  `last_schedule_time` varchar(200) COLLATE utf8_bin DEFAULT NULL,
  `network_name` varchar(200) COLLATE utf8_bin DEFAULT NULL,
  `network_ips` varchar(200) COLLATE utf8_bin DEFAULT NULL,
  `job_completions` varchar(200) COLLATE utf8_bin DEFAULT NULL,
  `script_id` int(11) DEFAULT NULL COMMENT '脚本id',
  `node_count` int(11) DEFAULT NULL COMMENT '节点数',
  `var` blob COMMENT 'vars.yml内容',
  `product_path` varchar(255) COLLATE utf8_bin DEFAULT NULL COMMENT '制品路径',
  `script_path` varchar(255) COLLATE utf8_bin DEFAULT NULL COMMENT '脚本路径',
  `inventory` blob COMMENT 'inventory',
  `node_group_id` int(11) DEFAULT NULL COMMENT '节点组id',
  PRIMARY KEY (`id`),
  UNIQUE KEY `application_index` (`project_id`,`application_name`,`platform_type`,`real_name`,`create_time`)
) ENGINE=InnoDB AUTO_INCREMENT=891 DEFAULT CHARSET=utf8 COLLATE=utf8_bin;
...
CREATE TABLE `v2_container` (
  ...
  `pod_ip` varchar(255) COLLATE utf8_bin DEFAULT NULL,
  `containerid` varchar(255) COLLATE utf8_bin DEFAULT NULL,
  `partition_name` varchar(255) COLLATE utf8_bin DEFAULT NULL,
  PRIMARY KEY (`id`),
...
CREATE TABLE `v2_pod` (
  ...
  `partition_name` varchar(255) COLLATE utf8_bin DEFAULT NULL,
  `pod_ip` varchar(255) COLLATE utf8_bin DEFAULT NULL,
  `restart_count` varchar(255) COLLATE utf8_bin DEFAULT NULL,
  `env_id` int(11) DEFAULT NULL,
  PRIMARY KEY (`id`),
  UNIQUE KEY `pod_index` (`image_group_id`,`namespace`,`real_name`,`kind`,`env_id`)
...
CREATE TABLE `node` (
  ...
  `node_ip` varchar(100) DEFAULT NULL COMMENT '节点ip',
  `node_status` tinyint(4) DEFAULT NULL COMMENT '节点状态（1.正常、0.异常）',
  `deploy_status` tinyint(4) DEFAULT NULL COMMENT '部署状态(1.已部署、0.未部署)',
  `node_cpu` varchar(255) DEFAULT NULL COMMENT 'CPU',
  `node_memory` varchar(255) DEFAULT NULL COMMENT '内存',
  `operate_system` varchar(255) DEFAULT NULL COMMENT '操作系统',
  `kernel_version` varchar(255) DEFAULT NULL COMMENT '操作系统',
  `script_id` int(11) DEFAULT NULL COMMENT '脚本id',
  `create_time` datetime DEFAULT NULL COMMENT '创建时间',
  `node_port` int(11) DEFAULT NULL COMMENT '端口',
  `delete_status` tinyint(4) DEFAULT '0' COMMENT '逻辑删除',
  PRIMARY KEY (`id`)
...
CREATE TABLE `node_group` (
  `id` int(11) NOT NULL AUTO_INCREMENT COMMENT '节点组id',
  `name` varchar(255) DEFAULT NULL COMMENT '节点组名称',
  `deploy_status` tinyint(4) DEFAULT NULL COMMENT '状态(1已部署、0未部署)',
  `create_time` datetime DEFAULT NULL COMMENT '创建时间',
  `delete_status` tinyint(4) DEFAULT '0' COMMENT '删除标记',
  `env_id` int(11) DEFAULT NULL,
  PRIMARY KEY (`id`)
...
CREATE TABLE `application_node` (
  `id` int(11) NOT NULL AUTO_INCREMENT COMMENT '服务节点id',
  `project_id` int(11) DEFAULT NULL COMMENT '应用id',
  `application_id` int(11) NOT NULL COMMENT '服务id',
  `node_group_id` int(11) NOT NULL COMMENT '节点组id',
  `node_id` int(11) NOT NULL COMMENT '节点id',
  `node_ip` varchar(100) NOT NULL COMMENT '节点ip',
  `label_id` int(11) DEFAULT NULL COMMENT '标签id',
  `label_name` varchar(255) DEFAULT NULL COMMENT '标签名称',
  `create_time` datetime DEFAULT NULL COMMENT '创建时间',
  `health_check_port` int(11) DEFAULT NULL COMMENT '健康检查路径',
  `health_check_path` varchar(100) DEFAULT NULL COMMENT '健康检查端口',
  `health_check_type` varchar(100) DEFAULT NULL COMMENT '健康检查类型',
  PRIMARY KEY (`id`)
...
DELIMITER ;;
CREATE DEFINER=`bocloud`@`%` PROCEDURE `checkProjectCore`(IN namespace varchar(200),IN cluster_id int(11), OUT project_id int(11),OUT env_id int(11),OUT env_name varchar(255))
BEGIN
 SET env_id = NULL;
 set project_id = NULL;
 set env_name = NULL;
 SELECT t.project_id,t.project_runtime_id,t.env_name INTO project_id,env_id,env_name
...
DELIMITER ;;
CREATE DEFINER=`bocloud`@`%` PROCEDURE `deleteProjectCore`(IN namespace varchar(200),IN cluster_id int(11),OUT result int(11))
BEGIN
    SELECT project_id into result FROM project_core WHERE project_core.project_name = namespace and project_core.cluster_id = cluster_id AND project_core.project_delete_datetime IS NULL;
...
BEGIN
	...
	IF ig_id IS NOT NULL THEN
		UPDATE v2_image_group SET v2_image_group.delete_time = NOW(),v2_image_group.`status` = "deleted"  WHERE v2_image_group.id = ig_id;
...
BEGIN
  SELECT t.id INTO application_id FROM v2_application t WHERE t.project_id = project_id AND t.kind = kind AND t.application_name = controller_name AND t.delete_time IS NULL;
END
;;
DELIMITER ;
...
DELIMITER ;;
CREATE DEFINER=`bocloud`@`%` PROCEDURE `insertOrGetProjectCore`(IN namespace varchar(200),IN project_desc text,IN cluster_id int(11),IN default_env_id int(11), IN env_id int(11),IN createtime datetime,IN default_env_name varchar(255),IN env_name varchar(255),IN projectid int(11),IN partition_id int(11),IN default_partition_id int(11),IN partition_name varchar(255),IN default_partition_name varchar(255),OUT project_id int(11))
BEGIN
	DECLARE real_env int(11) DEFAULT default_env_id;
	DECLARE real_env_name varchar(255) DEFAULT default_env_name;
	DECLARE real_partition_id int(11) DEFAULT default_partition_id;
  DECLARE real_partition_name varchar(255) DEFAULT default_partition_name;
	IF env_id IS NOT NULL THEN
		set real_env = env_id;
	END IF;
	IF env_name IS NOT NULL THEN
		set real_env_name = env_name;
	END IF;
	IF partition_id IS NOT NULL THEN
		set real_partition_id = partition_id;
	END IF;
	IF partition_name IS NOT NULL THEN
		set real_partition_name = partition_name;
	END IF;
	SET project_id = NULL;
  ...
  IF project_id IS NULL THEN
		INSERT INTO project_core (project_id,project_name,project_description,project_runtime_id,project_comments,project_create_datetime,project_delete_datetime,project_current_status,project_namespace,cluster_id,env_name,partition_id,partition_name,project_type)
		VALUES(projectid,namespace,project_desc,real_env,NULL,createtime ,NULL,"created",namespace,cluster_id,real_env_name,real_partition_id,real_partition_name,"2");
  SELECT t.project_id into project_id from project_core t WHERE t.project_name = namespace and t.cluster_id = cluster_id and t.project_delete_datetime IS NULL;
	END IF;
END
;;
...
DELIMITER ;;
CREATE DEFINER=`bocloud`@`%` PROCEDURE `insertOrUpdateApplication`(IN project_id int(11),IN controller_name varchar(200),IN platform varchar(200),IN affair_type varchar(200),IN micro_service_status varchar(200),IN micro_service_type varchar(200),IN monitor_status varchar(200),IN apollo_status varchar(200),IN create_time datetime,IN kind varchar(200),IN schedule varchar(200),IN last_schedule_time varchar(200),IN network_name varchar(200),IN network_ips varchar(200),IN job_completions varchar(200),OUT application_id int(11))
BEGIN
  SELECT t.id INTO application_id FROM v2_application t WHERE t.project_id = project_id AND t.kind = kind AND t.application_name = controller_name AND t.delete_time IS NULL;
	IF application_id IS NOT NULL THEN
		UPDATE v2_application t SET t.update_time = NOW(),t.micro_service_status = micro_service_status,t.monitor_status = monitor_status,t.apollo_status=apollo_status,t.last_schedule_time = last_schedule_time,t.network_name = network_name,t.network_ips = network_ips,t.job_completions=job_completions WHERE t.id = application_id;
	END IF;

		IF application_id IS NULL THEN
		INSERT INTO v2_application (project_id,application_name,url,description,real_name,platform_type,affair_type,module_type,create_time,update_time,delete_time,`status`,micro_service_status,micro_service_type,monitor_status,apollo_status,kind,schedule,last_schedule_time,network_name,network_ips,job_completions)
		VALUES(project_id,controller_name,NULL,NULL,controller_name,platform,affair_type,NULL,create_time,NOW(),NULL,"created",micro_service_status,micro_service_type,monitor_status,apollo_status,kind,schedule,last_schedule_time,network_name,network_ips,job_completions);
...
BEGIN
	...
	IF container_id is NOT NULL THEN
		UPDATE v2_container SET v2_container.`status` = container_status,v2_container.start_datetime = start_datetime,v2_container.pod_name = pod_name,v2_container.container_name = container_name,v2_container.real_name= real_name,v2_container.host_ip = host_ip,v2_container.host_name=host_name,v2_container.pod_ip=pod_ip,v2_container.containerid=containerid,v2_container.partition_name=partition_name,v2_container.uuid=uuid WHERE v2_container.id = container_id;
	END IF;

	IF container_id is NULL THEN
		INSERT INTO v2_container
		 (id,pod_id,image_group_id,version_id,application_id,project_id,env_id,pod_name,real_name,container_name,image_name_tag,status,limit_cpu,limit_memory,request_cpu,request_memory,host_ip,host_name,create_datetime,start_datetime,project_name,uuid,pod_ip,containerid,partition_name) VALUES(NULL,pod_id,image_group_id,version_id,application_id,project_id,env_id,pod_name,real_name,container_name,image_name_tag,container_status,limit_cpu,limit_memory,request_cpu,request_memory,host_ip,host_name,create_datetime,start_datetime,project_name,uuid,pod_ip,containerid,partition_name);
...
-- ----------------------------
-- Procedure structure for updateApplication
-- ----------------------------
DROP PROCEDURE IF EXISTS `updateApplication`;
DELIMITER ;;
CREATE DEFINER=`bocloud`@`%` PROCEDURE `updateApplication`(IN project_id int(11),IN controller_name varchar(200),IN platform varchar(200),IN affair_type varchar(200),IN micro_service_status varchar(200),IN micro_service_type varchar(200),IN monitor_status varchar(200),IN apollo_status varchar(200),IN create_time datetime,IN kind varchar(200),IN schedule varchar(200),IN last_schedule_time varchar(200),IN network_name varchar(200),IN network_ips varchar(200),IN job_completions varchar(200),OUT application_id int(11))
BEGIN
...
-- ----------------------------
-- Procedure structure for updateImageGroup
-- ----------------------------
DROP PROCEDURE IF EXISTS `updateImageGroup`;
DELIMITER ;;
CREATE DEFINER=`bocloud`@`%` PROCEDURE `updateImageGroup`(IN version_id int(11),IN application_id int(11),IN project_id int(11),IN env_id int(11),IN base_name varchar(200),IN real_name varchar(200),IN uuid varchar(200),IN namespace varchar(200),IN apiversion varchar(200),IN kind varchar(200),IN replica int(11),IN app_name varchar(200),IN ip_host varchar(200),IN if_http varchar(20),IN api_port varchar(20),IN createtime datetime,IN image_group_strategy varchar(255),IN shrinkage_image_group_id int(11), IN target_replica int(11) ,IN old_replica int(11) , OUT ig_id int(11))
BEGIN
	set ig_id = NULL;
	select t.id into ig_id FROM v2_image_group t where t.version_id = version_id AND t.application_id = application_id and t.project_id = project_id and t.env_id = env_id and t.delete_time IS NULL;
	IF ig_id is NOT NULL THEN
...
DELIMITER ;;
CREATE DEFINER=`bocloud`@`%` PROCEDURE `insertOrUpdatePod`(IN image_group_id int(11),IN uuid varchar(255),IN kind varchar(255),IN parent_name varchar(255),IN real_name varchar(255),IN namespace varchar(255),IN api_version varchar(255),IN resource_version varchar(255),IN update_time datetime,IN pod_status varchar(255),IN partition_name varchar(255),IN pod_ip varchar(255),IN restart_count varchar(255),IN env_id int(11),OUT pod_id int(11))
BEGIN
	...
	if pod_id IS NOT NULL AND com_res = 1 THEN
		UPDATE v2_pod set v2_pod.`status` = pod_status,v2_pod.uuid=uuid,v2_pod.resource_version = resource_version,update_time=update_time,pod_ip = pod_ip,partition_name=partition_name,restart_count=restart_count WHERE v2_pod.id = pod_id;
	END IF;

	if pod_id IS NULL THEN
		INSERT INTO v2_pod VALUES(NULL,uuid,image_group_id ,NULL,kind ,NULL,NULL,NULL,parent_name ,real_name,namespace,api_version,resource_version,update_time,pod_status,partition_name,pod_ip,restart_count,env_id);
